...
void Matrix::getData(double* val_, int i1, int j1, int i2, int j2) {
	if (i2 == -1) i2 = m - 1;
	if (j2 == -1) j2 = n - 1;
	int k = 0;
	for (int i = i1; i <= i2; i++)
...
void Matrix::setVal(double s, int i1, int j1, int i2, int j2) {
	if (i2 == -1) i2 = m - 1;
	if (j2 == -1) j2 = n - 1;
	if (i2 < i1 || j2 < j1) {
...
void Matrix::setDiag(double s, int i1, int i2) {
...
Matrix Matrix::reshape(const Matrix &M, int m_, int n_) {
	if (M.m*M.n != m_*n_) {
...
bool Matrix::lu(int *idx, double &d, double eps) {
